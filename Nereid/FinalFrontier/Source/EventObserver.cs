      ...
      class EventObserver
      {
         ...
         private void OnLaunch(EventReport report)
         {
            ...
            if(vessel!=null)
            {
               ...
               if (vessel.mainBody != null && vessel.mainBody.IsKerbin() && vessel.IsLanded())
               {
         ...
         {
            if (Log.IsLogable(Log.LEVEL.DETAIL)) Log.Detail("reset of inspectors");
            machInspector.Reset();
         ...
         {
            if (Log.IsLogable(Log.LEVEL.DETAIL)) Log.Detail("clear of inspectors");
            machInspector.Clear();
