   ...
   namespace FinalFrontier
   {
      static class Persistence
      {
         ...
         public static void Save(Game game, HallOfFame hallOfFame)
         {
            ...
            int numberOfBackupFiles = FinalFrontier.configuration.GetNumberOfBackupFiles();
            if (numberOfBackupFiles > 0)
            {
               Utils.FileRotate(filename, numberOfBackupFiles);
            }
            else 
            {
               Log.Info("file rotation for hall of fame disabled");
            }
            //
         ...
         public static List<LogbookEntry> LoadLogbook(String filename, double timeLimit=0)
         {
            ...
            try
            {
               ...
               {
                  Log.Warning("no final frontier logbook (" + filename + ") found");
                  return book;
               }
               ...
               while ((line = file.ReadLine()) != null)
               {
                  ...
                  if (line.Trim().Length > 0)
                  {
                     ...
                     if(entry!=null)
                     {
                        Log.Trace("hall of fame entry deserialized");
                        if (timeLimit == 0 || entry.UniversalTime <= timeLimit)
                        {
                           Log.Trace("hall of fame entry before current game time");
                           book.Add(entry);
                        }
                        else
                        {
            ...
            finally
            {
               ...
               Log.Detail("loading logbook complete");
            }
            return book;
         }
