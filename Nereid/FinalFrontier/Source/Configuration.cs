...
namespace Nereid
{
   namespace FinalFrontier
   {
      public class Configuration
      {
         ...
         private int numberOfBackupFiles = 9;
         ...
         public int GetNumberOfBackupFiles()
         {
            return numberOfBackupFiles;
         }

         public void SetNumberOfBackupFiles(int n)
         {
            numberOfBackupFiles = n;
         }
         ...
         public void Save()
         {
            ...
            try
            {
               using (BinaryWriter writer = new BinaryWriter(File.Open(filename, FileMode.Create)))
               {
                  ...
                  //
                  writer.Write((UInt16)numberOfBackupFiles);
               }
            }
            catch
            {
            ...
            try
            {
               if (File.Exists(filename))
               {
                  ...
                  using (BinaryReader reader = new BinaryReader(File.OpenRead(filename)))
                  {
                     ...
                     //
                     numberOfBackupFiles = reader.ReadUInt16();
                  }
               }
               else
               {
