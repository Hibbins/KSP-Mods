   ...
   namespace FinalFrontier
   {
      class ConfigWindow : PositionableWindow
      {
         ...
         protected override void OnWindow(int id)
         {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("Number of backup files:", FFStyles.STYLE_LABEL);
            String numberOfBackupFiles = config.GetNumberOfBackupFiles().ToString();
            numberOfBackupFiles = GUILayout.TextField(numberOfBackupFiles, STYLE_TEXTFIELD_WIDOWTITLE);
            try { config.SetNumberOfBackupFiles(int.Parse(numberOfBackupFiles)); }
            catch (FormatException e) { config.SetNumberOfBackupFiles(0);  }
            GUILayout.EndHorizontal();
            //
